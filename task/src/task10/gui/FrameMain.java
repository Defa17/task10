...
import cs.util.JTableUtils;
import cs.util.SwingUtils;
import task10.logic.Tablet;
import task10.logic.TabletChoice;
import task10.utils.InputFileReaderTablets;
import task10.utils.OutputFileWriterTablets;
        ...
        necessaryRate.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent actionEvent) {
                ...
                try {
                    ...
                    Integer.parseInt(necessaryRate.getText());
        ...
        executeButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent actionEvent) {
                try {
                    ...
                    int rate = Integer.parseInt(necessaryRate.getText());
                    int quantityOfTabletsInt = Integer.parseInt(quantityOfTablets.getText());


                    List<Tablet> bestTablet = TabletChoice.listOfCheapTablets(listOfTablets, memory, rate, quantityOfTabletsInt);
                    double sumTablets = TabletChoice.price(listOfTablets, memory, rate, quantityOfTabletsInt);

                    if ((arrayOfLines.length != 0) && (bestTablet.size() >= quantityOfTabletsInt)) {
                        answerTextArea.setText("Выбранные планшеты: " + bestTablet + "\nСтоимость:" + sumTablets);
                    } else {
                        answerTextArea.setText("Ошибка, пожалуйста, проверьте данные");
                    }
                } catch (Exception e) {
                    SwingUtils.showInfoMessageBox("Entrance data error");
                }
            ...
            public void actionPerformed(ActionEvent actionEvent) {
                try {
                    if (FILE_CHOOSER_SAVE.showSaveDialog(panelMain) == JFileChooser.APPROVE_OPTION) {
                        ...
                        int memory = Integer.parseInt(memorySize.getText());
                        int rate = Integer.parseInt(necessaryRate.getText());
                        int quantity = Integer.parseInt(quantityOfTablets.getText());
                        List<Tablet> bestTablet = tabletChoice.listOfCheapTablets(listOfTablets, memory, rate, quantity);
